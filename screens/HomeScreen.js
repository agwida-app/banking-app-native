...
function exportCSV(clients) {
 ...
 Share.share({ message: csv, title: 'قائمة العملاء.csv' })
...
function exportPDF(clients, single = null) {
 ...
 let text = `💳 ${title}\n`;
 text += `📅 التاريخ: ${date} | الإجمالي: ${list.length} عميل\n`;
 text += '═'.repeat(40) + '\n\n';
 ...
 list.forEach((c, i) => {
   ...
   text += `${i + 1}. ${c.name || '—'}\n`;
   text += `   🏦 المصرف: ${bank || '—'}\n`;
   text += `   📱 الهاتف: ${c.phone1 || '—'}\n`;
   text += `   🪪 الرقم الوطني: ${c.nationalId || '—'}\n`;
   if (c.iban) text += `   🏧 IBAN: ${c.iban}\n`;
   if (c.accountNumber) text += `   💳 رقم الحساب: ${c.accountNumber}\n`;
   if (c.amount) text += `   💰 المبلغ: ${parseFloat(c.amount).toLocaleString('en')} ${c.currency}\n`;
   if (c.paymentType) text += `   💵 نوع الحجز: ${c.paymentType}\n`;
   text += `   📋 الحالة: ${status}\n`;
   if (c.bookingDate) text += `   📅 تاريخ الحجز: ${c.bookingDate}\n`;
   if (c.pinCode) text += `   🔐 الرقم السري: ${c.pinCode}\n`;
   if (c.soldTo) text += `   👤 بيعت إلى: ${c.soldTo}\n`;
   if (c.purchasedBy) text += `   👤 اشترى من طرف: ${c.purchasedBy}\n`;
   if (c.notes) text += `   📝 ملاحظات: ${c.notes}\n`;
   text += '─'.repeat(30) + '\n';
 });

 text += `\nتطبيق إدارة بطاقاتك | ${date}`;

 Share.share({ message: text, title: title })
   .catch(e => Alert.alert('خطأ', e.message));
}
...
export default function HomeScreen({ subDays, navigation }) {
 ...
 const handleCancel = (isDirty) => {
   if (isDirty) {
     Alert.alert('إلغاء', 'هل تريد الخروج؟ سيتم فقدان التغييرات.', [
       { text: 'استمرار التعديل', style: 'cancel' },
 ...
 const viewRows = sel ? [
   { l: 'الاسم الكامل', v: sel.name, copy: false },
   { l: 'المصرف', v: sel.bankType === 'أخرى' ? sel.bankTypeOther : sel.bankType, copy: false },
   { l: 'الهاتف 1', v: sel.phone1, copy: true },
   { l: 'الهاتف 2', v: sel.phone2 || '—', copy: true },
   { l: 'الرقم الوطني', v: sel.nationalId, copy: true },
   { l: 'جواز السفر', v: sel.passportId || '—', copy: true },
   { l: 'رقم الحساب', v: sel.accountNumber || '—', copy: true },
   { l: 'IBAN', v: sel.iban || '—', copy: true },
   { l: 'المبلغ', v: sel.amount ? `${parseFloat(sel.amount).toLocaleString('en')} ${sel.currency}` : '—', copy: false },
   { l: 'تم الشراء من طرف', v: sel.purchasedBy || '—', copy: false },
   { l: 'نوع الحجز', v: sel.paymentType || '—', copy: false },
   { l: 'حالة البطاقة', v: sel.cardBooked ? '✅ تم الحجز' : '⏳ لم يتم', copy: false },
   { l: 'تاريخ الحجز', v: sel.bookingDate || '—', copy: false },
   { l: 'الرقم السري', v: sel.pinCode || '—', copy: true },
   { l: 'حالة البيع', v: sel.isSold ? '🔴 تم البيع' : '🟢 لم يُباع', copy: false },
   { l: 'بيعت إلى', v: sel.soldTo || '—', copy: false },
   { l: 'ملاحظات', v: sel.notes || '—', copy: false },
   { l: 'تاريخ الإضافة', v: fmt(sel.createdAt), copy: false },
   { l: 'أضيف بواسطة', v: sel.createdBy || '—', copy: false },
 ] : [];
 ...
 return (
   <View style={s.wrap}>
     ...
     <Modal visible={modal === 'view'} animationType="slide">
       <View style={s.modalWrap}>
         ...
         <ScrollView style={{flex:1}}>
           {/* بيانات منظمة في بطاقات */}
           <View style={s.viewSection}>
             <Text style={s.viewSectionTitle}>📋 البيانات الأساسية</Text>
             {viewRows.slice(0, 4).map(({ l, v, copy: canCopy }) => (
               <View key={l} style={s.viewRow}>
                 <Text style={s.viewLabel}>{l}</Text>
                 <View style={s.viewValWrap}>
                   <Text style={s.viewVal}>{v}</Text>
                   {canCopy && v !== '—' && (
           ...
           <View style={s.viewSection}>
             <Text style={s.viewSectionTitle}>🏦 البيانات المصرفية</Text>
             {viewRows.slice(4, 8).map(({ l, v, copy: canCopy }) => (
               <View key={l} style={s.viewRow}>
                 <Text style={s.viewLabel}>{l}</Text>
                 <View style={s.viewValWrap}>
                   <Text style={[s.viewVal, {fontFamily:'monospace', fontSize:12}]}>{v}</Text>
                   {canCopy && v !== '—' && (
                     <TouchableOpacity onPress={() => copy(v, l)} style={s.copyBtn}>
                       <Text style={s.copyBtnT}>📋</Text>
                     </TouchableOpacity>
           ...
           <View style={s.viewSection}>
             <Text style={s.viewSectionTitle}>💳 بيانات البطاقة</Text>
             {viewRows.slice(8, 15).map(({ l, v, copy: canCopy }) => (
               <View key={l} style={s.viewRow}>
                 <Text style={s.viewLabel}>{l}</Text>
                 <View style={s.viewValWrap}>
                   <Text style={s.viewVal}>{v}</Text>
           ...
           <View style={s.viewSection}>
             <Text style={s.viewSectionTitle}>📝 معلومات إضافية</Text>
             {viewRows.slice(15).map(({ l, v }) => (
               <View key={l} style={s.viewRow}>
                 <Text style={s.viewLabel}>{l}</Text>
                 <Text style={s.viewVal}>{v}</Text>
               </View>
     ...
     <Modal visible={bankModal} animationType="slide" transparent>
       <View style={s.bankModalOverlay}>
         <View style={s.bankModalWrap}>
           ...
           <ScrollView>
             {BANKS.map(b => (
               <TouchableOpacity key={b} style={[s.bankOption, form.bankType === b && s.bankOptionOn]}
     ...
     <Modal visible={modal === 'form'} animationType="slide">
       <View style={s.modalWrap}>
         <View style={s.modalHead}>
           <TouchableOpacity onPress={() => handleCancel(JSON.stringify(form) !== JSON.stringify(sel || EMPTY))}>
             <Text style={s.modalClose}>✕</Text>
           </TouchableOpacity>
         ...
         <ScrollView style={{padding:16}}>
           ...
           <View style={s.fg}>
             ...
             <TouchableOpacity style={[s.input, {justifyContent:'space-between', flexDirection:'row', alignItems:'center', paddingVertical:14}]}
         ...
         <View style={s.modalFoot}>
           <TouchableOpacity style={s.cancelBtn}
             onPress={() => handleCancel(JSON.stringify(form) !== JSON.stringify(sel || EMPTY))}>
...
const s = StyleSheet.create({
 ...
 viewSection:     { backgroundColor:'#0f2040', borderRadius:12, margin:14, marginBottom:8, borderWidth:1, borderColor:'rgba(201,168,76,0.15)', overflow:'hidden' },
 ...
 viewVal:         { fontSize:13, color:'#f8f6f0', fontWeight:'500', textAlign:'left', flex:1 },
