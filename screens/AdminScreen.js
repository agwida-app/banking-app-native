        ...
        loading
          ...
          : <FlatList
              ...
              renderItem={({ item: sub }) => {
                ...
                const deviceCount = Object.keys(sub.devices || {}).length;
                return (
                  <View style={s.card}>
                    ...
                    <Text style={s.meta}>📱 الأجهزة: {deviceCount}/7</Text>
          ...
          renderItem={({ item: aff }) => {
            ...
            return (
              <View style={s.card}>
                ...
                <Text style={s.meta}>💵 المدفوع: ${totalPaid.toLocaleString()}</Text>
                    ...
                    {affPays.length === 0
                      ...
                      : affPays.map(p => (
                          <View key={p.id} style={{flexDirection:'row', justifyContent:'space-between', padding:8, backgroundColor:'rgba(255,255,255,0.03)', borderRadius:8, marginBottom:4}}>
                            <Text style={{color:'#2ecc71', fontSize:13, fontWeight:'700'}}>${(p.amount||0).toLocaleString()}</Text>
                            <Text style={{color:'#8a9ab5', fontSize:11}}>{p.date} · {p.note}</Text>
                          </View>
                        ))
                    }
                    <Text style={[s.meta, {color:'#c9a84c', marginTop:6}]}>المجموع: ${totalPaid.toLocaleString()}</Text>
                  </View>
                )}
              </View>
            );
      ...
      <Modal visible={modal === 'sub'} animationType="slide">
        <View style={s.modalWrap}>
          ...
          <ScrollView style={{padding:16}}>
            ...
            {form.plan && (() => {
              const plan = PLANS.find(p => p.id === form.plan);
              const exp = addTime(plan.months, plan.days);
              return (
                <View style={{backgroundColor:'rgba(201,168,76,0.07)', borderRadius:8, padding:10, marginBottom:14}}>
                  <Text style={{color:'#2ecc71', fontSize:12}}>✅ ينتهي في: {exp.toLocaleDateString('ar-LY')}</Text>
                  <Text style={{color:'#c9a84c', fontSize:12, marginTop:3}}>💵 السعر: ${plan.price}</Text>
                </View>
              );
            })()}
